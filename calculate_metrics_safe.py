...
def get_euc_distances(zscores):
	...
	# here we need to check the entire interval for each location, check dimensions and then do the comparison on the join of these
...
def get_zscore_comparison(df):
	...
	for dim in dims:
		...
		if dimension_df.count() > 1:	
			...
			print(mean, stddev)
			dim_zscores = dimension_df.rdd.map(lambda row: ((row[2]), (row[5] if stddev == 0 else (float(row[5]) - mean) / stddev)  ))
			#dim_zscores = dimension_df.rdd.map(lambda row: ((row[2] ),(row[3],dim.dimension_labels,(row[5]) if stddev == 0 else (float(row[5]) - mean) / stddev) )).groupByKey().map(lambda x: (x[0], list(x[1]))) 
...
# 
# So we only need to compare within in time 
...
def compare_locations(df, ref_locale, locale_class, sc,dim_instances = 'all', interval = 'all'):
	...
	location_distances = get_euc_distances(zscores)
